...
pub fn uringlator_start(self: *@This(), op: anytype, id: u16) !void {
    switch (comptime Operation.tagFromPayloadType(@TypeOf(op.*))) {
        ...
        .read => {
            ...
            const ret = wtry(windows.kernel32.ReadFile(h.?, op.buffer.ptr, @intCast(op.buffer.len), &read, &self.ovls[id].overlapped)) catch |err| return self.uringlator.finish(self, id, err, .thread_unsafe);
            if (ret != 0) {
        ...
        .write => {
            ...
            const ret = wtry(windows.kernel32.WriteFile(h.?, op.buffer.ptr, @intCast(op.buffer.len), &written, &self.ovls[id].overlapped)) catch |err| return self.uringlator.finish(self, id, err, .thread_unsafe);
            if (ret != 0) {
...
pub fn uringlator_complete(self: *@This(), op: anytype, id: u16, failure: Operation.Error) void {
    ...
    if (failure == error.Success) {
        ...
        switch (comptime Operation.tagFromPayloadType(@TypeOf(op.*))) {
            .timeout, .link_timeout => self.tqueue.disarm(.monotonic, id),
            .wait_event_source => op.source.native.removeWaiter(&op._.link),
            .accept => if (op.out_socket.* != INVALID_SOCKET) checked(windows.ntdll.NtClose(op.out_socket.*) == .SUCCESS),
            else => {},
        }
    }
}
