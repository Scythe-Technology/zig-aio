...
pub fn uringlator_start(self: *@This(), op: anytype, id: u16) !void {
    ...
    switch (comptime Operation.tagFromPayloadType(@TypeOf(op.*))) {
        ...
        .read => {
            ...
            wtry(windows.kernel32.ReadFile(h.?, op.buffer.ptr, @intCast(op.buffer.len), &trash, &self.ovls[id].overlapped)) catch |err| return self.uringlator.finish(id, err, .thread_unsafe);
        },
        .write => {
            const flags = try getHandleAccessInfo(op.file.handle);
            ...
            self.iocp.associateHandle(id, h.?) catch |err| return self.uringlator.finish(id, err, .thread_unsafe);
            self.ovls[id] = .{ .overlapped = ovlOff(op.offset), .owned = .{ .handle = h.? } };
            wtry(windows.kernel32.WriteFile(h.?, op.buffer.ptr, @intCast(op.buffer.len), &trash, &self.ovls[id].overlapped)) catch |err| return self.uringlator.finish(id, err, .thread_unsafe);
        },
        .accept => {
            ...
            wtry(windows.ws2_32.AcceptEx(op.socket, op.out_socket.*, &op._, 0, @sizeOf(std.posix.sockaddr) + 16, @sizeOf(std.posix.sockaddr) + 16, &trash, &self.ovls[id].overlapped) == 1) catch |err| return self.uringlator.finish(id, err, .thread_unsafe);
        },
        .recv => {
            self.iocp.associateSocket(id, op.socket) catch |err| return self.uringlator.finish(id, err, .thread_unsafe);
